 ...
 * no description
 */
export class PersonInfoApiRequestFactory extends BaseAPIRequestFactory {
  /**
   * Info about logged in person.
   */
  public async personInfoControllerInfo(
...
export class PersonInfoApiResponseProcessor {
  /**
   * Unwraps the actual response sent by the server from the response context and deserializes the response content
   * to the expected objects
   *
   * @params response Response returned by the server for a request to personInfoControllerInfo
   * @throws ApiException if the response code was not in [200, 299]
   */
  public async personInfoControllerInfoWithHttpInfo(
    ...
    if (isCodeInRange("200", response.httpStatusCode)) {
      const body: PersonInfoResponse = ObjectSerializer.deserialize(
