...
export class ImportApiRequestFactory extends BaseAPIRequestFactory {
  ...
  public async importControllerDeleteImportTransaction(
    ...
    // Path Params
    const localVarPath = "/api/import/{importvorgangId}".replace(
      "{" + "importvorgangId" + "}",
      encodeURIComponent(String(importvorgangId)),
    );
   ...
   */
  public async importControllerExecuteImport(
    ...
    if (
      importvorgangByIdBodyParams === null ||
      importvorgangByIdBodyParams === undefined
    ) {
      throw new RequiredError(
        "ImportApi",
   ...
   * @param file
   */
  public async importControllerUploadFile(
    ...
    // Form Params
    const useForm = canConsumeForm(["multipart/form-data"]);
...
export class ImportApiResponseProcessor {
   ...
   */
  public async importControllerDeleteImportTransactionWithHttpInfo(
    ...
    if (isCodeInRange("400", response.httpStatusCode)) {
      ...
      throw new ApiException<DbiamImportError>(
        response.httpStatusCode,
        "Something went wrong with the found import transaction.",
        body,
        response.headers,
      );
    }
    if (isCodeInRange("401", response.httpStatusCode)) {
      throw new ApiException<undefined>(
        response.httpStatusCode,
        "Not authorized to delete the import transaction.",
        undefined,
        response.headers,
      );
    }
    if (isCodeInRange("404", response.httpStatusCode)) {
      throw new ApiException<undefined>(
        response.httpStatusCode,
        "The import transaction that should be deleted does not exist.",
        undefined,
        response.headers,
      );
    }
   ...
   * @throws ApiException if the response code was not in [200, 299]
   */
  public async importControllerExecuteImportWithHttpInfo(
    response: ResponseContext,
    ...
    if (isCodeInRange("401", response.httpStatusCode)) {
      throw new ApiException<undefined>(
        response.httpStatusCode,
        "Not authorized to execute the import transaction.",
        undefined,
        response.headers,
      );
    }
    if (isCodeInRange("403", response.httpStatusCode)) {
      throw new ApiException<undefined>(
        response.httpStatusCode,
        "Insufficient permissions to execute the import transaction.",
        undefined,
        response.headers,
      );
    }
    if (isCodeInRange("404", response.httpStatusCode)) {
      throw new ApiException<undefined>(
        response.httpStatusCode,
        "The import transaction does not exist.",
        undefined,
        response.headers,
      );
    }
    if (isCodeInRange("500", response.httpStatusCode)) {
      throw new ApiException<undefined>(
        response.httpStatusCode,
        "Internal server error while executing the import transaction.",
        undefined,
        response.headers,
      );
    }
   ...
   * @throws ApiException if the response code was not in [200, 299]
   */
  public async importControllerUploadFileWithHttpInfo(
    response: ResponseContext,
    ...
    if (isCodeInRange("401", response.httpStatusCode)) {
      throw new ApiException<undefined>(
        response.httpStatusCode,
        "Not authorized to import data with a CSV file.",
        undefined,
        response.headers,
      );
    }
    if (isCodeInRange("403", response.httpStatusCode)) {
      throw new ApiException<undefined>(
        response.httpStatusCode,
        "Insufficient permissions to import data with a CSV file.",
        undefined,
        response.headers,
      );
    }
    if (isCodeInRange("500", response.httpStatusCode)) {
      throw new ApiException<undefined>(
        response.httpStatusCode,
        "Internal server error while importing data with a CSV file.",
        undefined,
        response.headers,
      );
    }
