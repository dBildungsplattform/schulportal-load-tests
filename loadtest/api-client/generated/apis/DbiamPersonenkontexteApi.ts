...
export class DbiamPersonenkontexteApiRequestFactory extends BaseAPIRequestFactory {
   ...
   */
  public async dBiamPersonenkontextControllerCreatePersonenkontextMigration(
    ...
    if (
      dbiamPersonenkontextMigrationBodyParams === null ||
      dbiamPersonenkontextMigrationBodyParams === undefined
    ) {
   ...
   */
  public async dBiamPersonenkontextControllerFindPersonenkontextsByPerson(
    personId: string,
    _options?: Configuration,
  ): Promise<RequestContext> {
    let _config = _options || this.configuration;
    ...
    // Path Params
    const localVarPath = "/api/dbiam/personenkontext/{personId}".replace(
      "{" + "personId" + "}",
...
export class DbiamPersonenkontexteApiResponseProcessor {
   ...
   */
  public async dBiamPersonenkontextControllerCreatePersonenkontextMigrationWithHttpInfo(
    ...
    if (isCodeInRange("400", response.httpStatusCode)) {
      ...
      throw new ApiException<DbiamPersonenkontextError>(
        response.httpStatusCode,
        "The personenkontext could not be created, may due to unsatisfied specifications.",
        body,
        response.headers,
      );
    }
    if (isCodeInRange("401", response.httpStatusCode)) {
      throw new ApiException<undefined>(
        response.httpStatusCode,
        "Not authorized to create personenkontext.",
        undefined,
        response.headers,
      );
    }
    if (isCodeInRange("403", response.httpStatusCode)) {
      throw new ApiException<undefined>(
        response.httpStatusCode,
        "Insufficient permission to create personenkontext.",
        undefined,
        response.headers,
      );
    }
    if (isCodeInRange("500", response.httpStatusCode)) {
      throw new ApiException<undefined>(
        response.httpStatusCode,
        "Internal server error while creating personenkontext.",
        undefined,
        response.headers,
      );
    }
   ...
   * @throws ApiException if the response code was not in [200, 299]
   */
  public async dBiamPersonenkontextControllerFindPersonenkontextsByPersonWithHttpInfo(
    response: ResponseContext,
    ...
    if (isCodeInRange("200", response.httpStatusCode)) {
      const body: Array<DBiamPersonenkontextResponse> =
    ...
    if (isCodeInRange("401", response.httpStatusCode)) {
      throw new ApiException<undefined>(
        response.httpStatusCode,
        "Not authorized to get available personenkontexte.",
        undefined,
        response.headers,
      );
    }
    if (isCodeInRange("403", response.httpStatusCode)) {
      throw new ApiException<undefined>(
        response.httpStatusCode,
        "Insufficient permission to get personenkontexte for this user.",
        undefined,
        response.headers,
      );
    }
    if (isCodeInRange("500", response.httpStatusCode)) {
      throw new ApiException<undefined>(
        response.httpStatusCode,
        "Internal server error while getting personenkontexte.",
        undefined,
        response.headers,
      );
    }
